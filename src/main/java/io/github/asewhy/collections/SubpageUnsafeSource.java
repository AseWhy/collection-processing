...
public class SubpageUnsafeSource<T, K> extends UnsafeDatasource<UnsafeFunction<K, Collection<T>>, T> {
    private final Iterator<K> subSource;

    public SubpageUnsafeSource(@NotNull Iterable<K> subSource, UnsafeFunction<K, Collection<T>> function, Integer retryCount) {
    ...
    @Contract("_, _, _ -> new")
    public static <T, K> @NotNull SubpageUnsafeSource<T, K> of(@NotNull Iterable<K> subSource, UnsafeFunction<K, Collection<T>> function, Integer retryCount) {
        return new SubpageUnsafeSource<>(subSource, function, retryCount);
    }

    @Contract("_, _ -> new")
    public static <T, K> @NotNull SubpageUnsafeSource<T, K> of(@NotNull Iterable<K> subSource, UnsafeFunction<K, Collection<T>> function) {
